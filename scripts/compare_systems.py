...
def main():
    ...
    b1_line = (
        f"On this same slice of 25 questions, Chain of Custody (partial sample, only "
        ...
        if b1 is not None
        else "Chain of Custody has not been run on this bucket."
    )

    report = f"""# Results: comparing Best Effort, Chain of Custody, and Ground Truth
...
exact number or citation): Best Effort scored {a1}%, Ground Truth scored {c1}%.
Winner: {numeric_fact_winner_1}.

Bucket 3 (sparse cell questions, where the correct answer is that no
fixed number exists): Best Effort scored {a3}%, Ground Truth scored {c3}%.
...
Best Effort scored {a2}%, Ground Truth scored {c2}%. Winner: {comparison_winner}.
