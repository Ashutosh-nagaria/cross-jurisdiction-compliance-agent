# Results: comparing Best Effort, Chain of Custody, and Ground Truth
...
Best Effort is a baseline retrieval and answer system. Ground Truth is a
deterministic rule lookup table with an AI classification step in front
of it. Both have been run on the full 90 question eval. Chain of Custody is a
LangGraph agent with routing, retrieval, structured extraction, source
verification, and a simulated human approval step. Chain of Custody has only
been run on bucket 1 (25 questions) so far, see the note at the end of
this report for why.

## Accuracy per bucket, side by side

| Bucket | What it tests | Best Effort (full 90) | Ground Truth (full 90) | Chain of Custody (bucket 1 only, partial sample) |
|---|---|---|---|---|
...
Chain of Custody's column is a partial sample of 25 questions, not a full 90
question result, and should not be read as directly equivalent to the
Best Effort and Ground Truth columns next to it.

## Release gate targets (Best Effort and Ground Truth only)

| Release gate target | Best Effort | Ground Truth |
|---|---|---|
...
Neither system currently meets all three release gates. Best Effort meets
the bucket 4 target outright. Ground Truth comes closer on the other two
...
exact number or citation): Best Effort scored 36.0%, Ground Truth scored 72.0%.
Winner: Ground Truth.

Bucket 3 (sparse cell questions, where the correct answer is that no
fixed number exists): Best Effort scored 60.0%, Ground Truth scored 80.0%.
Winner: Ground Truth.

On this same slice of 25 questions, Chain of Custody (partial sample, only bucket 1 has been run) scored 24.0%, the lowest of the three, though it was tested under a harder five step pipeline with fewer rounds of tuning than the other two systems received.

Ground Truth wins clearly on both numeric fact buckets. This is expected
given how the two systems work. Ground Truth's answers for these 25
...
generation step to phrase the citation inconsistently. Best Effort has to
...
Best Effort scored 80.0%, Ground Truth scored 92.0%. Winner: Ground Truth.
...
Ground Truth's classification step simply returns multiple jurisdiction and
...
Best Effort has to retrieve, hold, and correctly restate several
...
Best Effort's advantage shows up on the buckets that ask for something a
...
and writing, not a lookup. Best Effort handled both of those well, hitting
100 percent on adversarial questions and 100 percent on drafting
questions, while Ground Truth's drafting answers were correct in substance
...
in the first place. Chain of Custody's design, verifying every claim against
...
## Why Chain of Custody's evaluation is partial

Chain of Custody was only run on bucket 1 in this project, not the full 90
question set. This is a budget and time constraint on this evaluation,
not a limitation found in Chain of Custody's design. Each full 90 question run
...
embeddings. Chain of Custody also went through two rounds of real bug fixes
...
on bucket 1, a full 90 question run for Chain of Custody had not yet been
carried out. The bucket 1 numbers already recorded reflect a fully
working version of Chain of Custody, not a broken one, they are simply a
smaller sample than Best Effort and Ground Truth received.
