...
None of that answers a separate, quieter question: is anyone actually using any of this, and where do they get stuck if they do? A rulebook, a well read assistant, and a supervised assistant are all worth nothing if nobody ever opens the door, or if people open it and give up before asking a real question. This project adds a small, honest instrument for exactly that, watching where visitors actually go: opening the app, asking a question, and seeing an answer come back. Not to report any real usage, since this is a portfolio project without a real audience, but to build the same kind of visibility a real deployment would need.
...
**Product analytics.** Knowing a system answers correctly is not the same as knowing whether anyone reaches an answer at all. The app sends four events to Mixpanel as someone moves through it: opening the app, submitting a question tagged with which of the three systems was asked, seeing an answer rendered, and, specific to Chain of Custody, rejecting an answer at the human approval step instead of approving it. The first three of those, opened the app, submitted a question, saw an answer, form a funnel, the same shape a real team would watch to see where visitors actually drop off. Events go to Mixpanel's EU endpoint rather than its default US one, since a project built around cross-border data transfer rules should think about where its own visitors' data goes too, not only the data it answers questions about.
